    ...
    return (
        <div>
            <div className='card relative'>
                <img className='card-img-fluid' width='200px' height='200px' src={props.itemImage} alt={Math.random().toString()}/>

                {cartAdded ? (
                    <span className='badge badge-pill badge-success position-absolute' style={{ right: 5, padding: 10, top: 5 }}>
                        <img alt={Math.random().toString()} width='20px' style={{ filter: 'invert()' }} height='20px' src={cartIcon}></img>
                    </span>
                ) : (
                    ''
                )}
                ...
                <div className='card-body'>
                    ...
                    <div className='row'>
                        <div className={cartAdded ? 'col-5 d-flex align-items-center' : 'col-4 d-flex align-items-center'}>
                            <div style={{ fontSize: '12px' }} onClick={cartAdded ? removeCardHandler : addToCartHandler} className={cartAdded ? 'btn btn-danger' : 'btn btn-success'}>
                                {cartAdded ? 'Remove from Cart' : 'Add to Cart'}
                            </div>
                        </div>
                        <div className='col-4 d-flex align-items-center justify-content-between'>
                            <button disabled={cartAdded} onClick={() => changeCount('+')}
                                    className={cartAdded ? 'border-0 rounded my-count-disabled' : 'border-0 rounded my-count bg-transparent'}>
                                +
                            </button>
